...
SWEP.Contact		= ""
SWEP.Purpose		= "Lich"
SWEP.Instructions	= "Left to Warp"
...
function SWEP:PrimaryAttack()
	location = self.Owner:GetPos()
	if SERVER then
		self.Portal = true
		self.Owner:ChatPrint("Warp location set. Use Secondary Fire to warp back to this location.")
	end
end
...
function SWEP:SecondaryAttack()
	if SERVER then 
		if self.Portal then
			...
			self.Owner:SetPos(Vector(location.x,location.y,location.z))
		else
			self.Owner:ChatPrint("No warp location set. Use Primary Fire to set a location.")
		end
	end
end
...
if CLIENT then -- Weapon HUD
	function SWEP:DrawHUD()
		...
		weapon = "Warp"
		desc = "Left to set Warp. Right to Warp"
