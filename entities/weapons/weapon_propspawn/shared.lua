...
SWEP.Contact		= ""
SWEP.Purpose		= "Lich"
SWEP.Instructions	= "Left to Select. Right to Spawn"
...
function SWEP:SecondaryAttack()
	if SERVER then
		...
		self.Prop = ent:GetModel()
		self.Owner:ChatPrint(tostring(self.Prop).." selected.")
	end
end
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		if self.Prop then
			...
			ent:SetModel(self.Prop)
			ent:SetPos(eyetrace.HitPos)
			ent:Spawn()
		else
			self.Owner:ChatPrint("No Prop Selected. Right Click to select a Prop to spawn")
		end
	end
end
...
function SWEP:Think()
	if self:Clip1() < 5 then
		self.NextAmmo = self.NextAmmo or CurTime() + 5
...
if CLIENT then -- Weapon HUD
	function SWEP:DrawHUD()
		...
		weapon = "Prop Spawner"
		desc = "Left to Spawn. Right to Select."
